...
class AuthService {
            ...
            User.logout { result in
                ...
                case .failure(let error as NSError):
                    if error.code == 209 {
                        ...
                        print(
                            "Erro ao deslogar usuário anterior: \(error.localizedDescription)"
                        )
    ...
    func recoverPassword(
        email: String, completion: @escaping (Bool, Error?) -> Void
    ) {
        User.passwordReset(email: email) { result in
            ...
            case .failure(let error):
                print(
                    "Erro ao enviar e-mail de recuperação: \(error.localizedDescription)"
                )
                completion(false, error)
            }
        }
